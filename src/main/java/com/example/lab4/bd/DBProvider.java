        ...
        while (res.next()) {
            ...
            if (res.getString(tabName + ".status").trim().equals("in operation")) {
                ...
                for (Reactor reactor : reactorArrayList) {
                    if (reactor.getName().equals(type)) { // если список из джсона совпадает с  типом из бд
                ...
                units.add(new Unit(id, name, type, thermalCapacity, loadFactor, burnup, siteID)); // обааляем  список все данные
            }
        }
    }
